...
namespace Nop.Plugin.Misc.Clickatell.EventConsumers
{
    public class OrderPlacedEventConsumer : IConsumer<OrderPlacedEvent>
    {
        ...
        public void HandleEvent(OrderPlacedEvent eventMessage)
        {
            ...
            {
                _logger.Error("Clickatell.Error.OrderPlacedEvent:" + ex.Message);
            }
        }
        async void SmsToAdmin(ClickatellPlugin plugin, Order order)
        {
            var adminNumber = FormatNumber(_clickatellSettings.PhoneNumber);
            var success = await plugin?.SendSms($"Hi Admin, new order #{order.Id} R{order.OrderTotal} has been PLACED", adminNumber);
        }
        async void SmsToCustomer(ClickatellPlugin plugin, Order order)
        {
            ...
            var success = await plugin?.SendSms($"Dear valued shopper, your order #{order.Id} of R{order.OrderTotal} on BuyBloem.com has been placed.", phone, "sms");
            if (success)
            {
                order.OrderNotes.Add(new OrderNote()
                {
                    Note = $"\"Order placed\" SMS alert {customer.Email} has been sent",
        ...
        async void SmsToVendors(ClickatellPlugin plugin, Order order)
        {
            ...
            {
                messages.Add(new { channel = "sms", content = $"Hello vendor, Order #{order.Id} has been placed. Items: {dictItem.Value}", to = dictItem.Key });
            }
            ...
            if (success)
            {
                order.OrderNotes.Add(new OrderNote()
                {
                    Note = $"\"Order placed\" SMS alert to vendors has been sent",
                    DisplayToCustomer = false,
                    CreatedOnUtc = DateTime.UtcNow
                });
                _orderService.UpdateOrder(order);
